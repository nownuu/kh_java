...
//= 전화를 걸기 위해서는 전화번호가 필요한 것처럼 소켓을 생성하려면 대상 프로그램의 위치정보가 필요하다.
//= IP정보와 Port정보가 필요하다.
public class Client {
...
//		Socket socket = new Socket(IP주소, Port번호);
		try {
			Socket socket = new Socket("172.30.1.12", 30000);//cmd에서 확인한 IP
//			Socket socket = new Socket("127.0.0.1", 30000);//127.0.0.1은 IP계의 this
